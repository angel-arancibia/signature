...
describe ApiSignature do
  before :each do
    ...
    @token = ApiSignature::Token.new('key', 'secret')

    @request = ApiSignature::Request.new('POST', '/some/path', {
      "query" => "params",
      "go" => "here"
    })
  end
  ...
  describe "verification" do
    ...
    it "should verify requests" do
      request = ApiSignature::Request.new('POST', '/some/path', @params)
    ...
    it "should validate that the provided token has a non-empty secret" do
      token = ApiSignature::Token.new('key', '')
      request = ApiSignature::Request.new('POST', '/some/path', @params)
    ...
    describe "authenticate_async" do
      ...
      it "returns a deferrable which fails if block df fails" do
        ...
        em {
          ...
          request_df.errback { |e|
            e.class.should == ApiSignature::AuthenticationError
      ...
      it "returns a deferrable which fails if request does not validate" do
        ...
        em {
          ...
          token = ApiSignature::Token.new('key', 'wrong_secret')
          df.succeed(token)
